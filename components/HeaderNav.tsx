...
const LINK_CLASS =
  "block rounded-xl px-3 py-2 text-stone-700 hover:bg-fuchsia-50 hover:text-[#ec1178] xl:px-2 xl:py-1";
const PRIMARY_LINK_CLASS =
  "block rounded-xl bg-[#ec1178] px-3 py-2 font-medium text-white shadow-sm hover:bg-[#c90e66] xl:rounded-full xl:py-1.5";
  ...
  return (
    <>
      <button
        ...
        className="inline-flex h-11 items-center justify-center gap-2 rounded-xl border border-stone-200 bg-white px-3 text-sm font-medium text-stone-700 shadow-sm hover:border-[#ec1178] hover:text-[#ec1178] xl:hidden"
      >
      ...
      <nav
        ...
        className={
          ...
          "xl:static xl:flex xl:max-h-none xl:flex-row xl:items-center xl:gap-2 xl:overflow-visible xl:border-0 xl:bg-transparent xl:p-0 xl:shadow-none " +
          "[&>button]:w-full [&>button]:justify-start [&>button]:rounded-xl [&>button]:px-3 [&>button]:py-2 [&>button]:text-left " +
          "xl:[&>button]:w-auto xl:[&>button]:rounded-lg xl:[&>button]:px-2 xl:[&>button]:py-1"
        }
      >
        <BuzonSugerencias />
        {authenticated ? (
          <>
            ...
            {email && <span className="hidden max-w-48 truncate text-stone-400 2xl:inline">{email}</span>}
            <LogoutButton />
          </>
        ) : (
          <>
